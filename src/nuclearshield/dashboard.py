...
def _masthead(s: FacilityState) -> Panel:
    ...
    meta.add_row(title, f"SIMULATION  {s.scenario.upper()}", f"[{risk}]CYBER POSTURE {s.risk_level}[/{risk}]  T+{s.tick:04d}")
...
def _containment_core(s: FacilityState) -> Panel:
    ...
    right.add_row("WRITE PATH", "[bold green]DISABLED[/bold green]")

    core = Text(justify="center")
    core.append("╭──────────────────╮\n", style="cyan")
    core.append("│ PROTECTED SAFETY │\n", style="bold white")
    core.append("│     ENVELOPE     │\n", style="bold cyan")
    core.append("╰────────┬─────────╯\n", style="cyan")
    core.append("         │ passive evidence\n", style="dim")
    core.append("         ▼", style="cyan")

    return Panel(
        Columns([left, Align.center(core), right], expand=True, equal=True),
        title="FACILITY CORE / SAFETY ENVELOPE",
...
def _perimeter(s: FacilityState) -> Panel:
    ...
    grid.add_row("ENTERPRISE", "DMZ", "OT-SCADA", "SAFETY", "PHYSICAL", "MC&A")
    grid.add_row(
        _lamp(s.enterprise_zone_ok),
        _lamp(s.dmz_zone_ok),
        _lamp(s.scada_zone_ok),
        _lamp(s.safety_zone_ok),
        _lamp(s.physical_security_ok),
        _lamp(s.material_accounting_ok),
    )
    grid.add_row("SOC", "broker", "observe", "independent", "access", "safeguards")
    return Panel(
        grid,
        title="PROTECTION RINGS",
        subtitle="segmented trust zones • one-way evidence paths • no control capability",
        box=box.SQUARE,
    )
...
def _mission_risk(s: FacilityState) -> Panel:
    ...
    body.append(f"FUSED RISK  {s.risk_level}\n\n", style=style)
    ...
    body.append(f"\nRESPONSE POSTURE: {s.response_mode}", style="bold cyan")
...
def _assurance(s: FacilityState) -> Panel:
    ...
    t.add_column()
    t.add_column(justify="right")
    t.add_row("IEC 62645", "CONCEPT MAPPED")
    t.add_row("NRC RG 5.71", "CONCEPT MAPPED")
    t.add_row("IAEA guidance", "CONCEPT MAPPED")
...
def _footer() -> Panel:
    return Panel(
        "[bold cyan]SIMULATION BOUNDARY[/bold cyan]  READ-ONLY • SYNTHETIC • DEFENSIVE   |   Real OT / physical access / SCADA: DISCONNECTED   |   Grafana :3000   Prometheus :9090   Metrics :9108   Ctrl+C EXIT",
        box=box.DOUBLE,
        padding=(0, 1),
    )
...
def render_dashboard(s: FacilityState) -> Layout:
    """NuclearShield's facility-protection command-center terminal."""
    layout = Layout()
    layout.split_column(
        Layout(name="masthead", size=3),
        Layout(name="core", size=9),
        Layout(name="rings", size=6),
        Layout(name="command", ratio=1, minimum_size=11),
        Layout(name="ticker", size=7),
        Layout(name="footer", size=3),
    )
    layout["command"].split_row(
        Layout(name="risk", ratio=5),
        Layout(name="safeguards", ratio=5),
        Layout(name="assurance", ratio=4),
    )
